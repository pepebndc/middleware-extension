...
function injectFeeIntoExecute(originalData, token, feeAmount) {
  try {
    ...
    // Safety: ensure commands and inputs are present and aligned; otherwise, do not modify
    if (!decoded.commands || decoded.commands === '0x') {
      console.log('⚠️ Empty commands in execute; skipping fee injection');
      return originalData;
    }
    const commandHex = decoded.commands.replace('0x', '');
    const commandBytes = commandHex.match(/.{1,2}/g) || [];
    if (commandBytes.length === 0 || !decoded.inputs || decoded.inputs.length === 0) {
      console.log('⚠️ No commands or inputs to preserve; skipping fee injection');
      return originalData;
    }
    if (commandBytes.length !== decoded.inputs.length) {
      console.log('⚠️ Commands/inputs length mismatch; skipping fee injection');
      return originalData;
    }
    
    // Insert TRANSFER before final payout operations (SWEEP/UNWRAP) to ensure fee is taken
    const payoutBytes = new Set(['04', '0c']);
    let insertIndex = commandBytes.findIndex((b) => payoutBytes.has(b.toLowerCase()));
    if (insertIndex === -1) insertIndex = commandBytes.length;

    const newBytes = [
      ...commandBytes.slice(0, insertIndex),
      TRANSFER_COMMAND.slice(2),
      ...commandBytes.slice(insertIndex)
    ];
    const newCommands = '0x' + newBytes.join('');
 ...
 * @param {bigint|string} amount - Amount as BigInt or hex/decimal string
 * @returns {string} - Full calldata starting with selector 0xa9059cbb
 */
function encodeErc20Transfer(to, amount) {
  const selector = '0xa9059cbb';
  const toBytes = padHexTo32Bytes(to);
  let amtBig;
  if (typeof amount === 'bigint') amtBig = amount; else if (typeof amount === 'string' && amount.startsWith('0x')) amtBig = BigInt(amount); else amtBig = BigInt(amount);
  const amtBytes = padHexTo32Bytes('0x' + amtBig.toString(16));
  return selector + toBytes + amtBytes;
}

/**
 * Encode multicall(uint256 deadline, bytes[] data) parameters
 * @param {string|bigint} deadline - Unix timestamp as decimal string or bigint
 * @param {string[]} calls - Array of hex strings (each starting with 0x) for bytes elements
 * @returns {string} - Encoded parameters WITHOUT 0x prefix
 */
function encodeMulticallParameters(deadline, calls) {
  const deadlineBig = typeof deadline === 'bigint' ? deadline : BigInt(String(deadline));
  const deadlineWord = padHexTo32Bytes('0x' + deadlineBig.toString(16));
  const arrayOffsetWord = padHexTo32Bytes('0x40');

  const arrayLengthWord = padHexTo32Bytes('0x' + calls.length.toString(16));

  const elementsData = calls.map((item) => {
    const clean = item.startsWith('0x') ? item.slice(2) : item;
    const lenWord = padHexTo32Bytes('0x' + (clean.length / 2).toString(16));
    const padded = clean + '0'.repeat((64 - (clean.length % 64)) % 64);
    return lenWord + padded;
  });

  let offset = calls.length * 64; // bytes
  const offsetsWords = elementsData.map((edata) => {
    const word = padHexTo32Bytes('0x' + offset.toString(16));
    offset += edata.length / 2; // hex chars -> bytes
    return word;
  });

  const arrayData = arrayLengthWord + offsetsWords.join('') + elementsData.join('');
  return deadlineWord + arrayOffsetWord + arrayData;
}
